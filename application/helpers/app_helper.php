<?php
	...
	function getUserLevel($role){
		...
		$CI->db->select('*');
		$CI->db->where('HEAD_CODE', 'APR01');
		$CI->db->where('CODE_NAME', $role);
		$query = $CI->db->get('CD_CODE');
	...
	function notifikasiWhatsapp($data) {
		if ($data['TYPE'] == 'APPROVAL_BROILER') {
			approvalBroilerWhatsapp($data);
		}
	}

	function approvalBroilerWhatsapp($data) {
		$CI =& get_instance();
		...
		$query_openprice = "
			SELECT
				...
				A.UNIT_PRICE,
				A.PRICE_AREA_CODE,
				D.CODE_NAME AS PRICE_AREA,
				B.CUSTOMER,
				C.FULL_NAME AS CUSTOMER_NAME,
				A.APPROVAL_LEVEL,
				A.CURRENT_APPROVAL_LEVEL,
				A.CREATED_AT AS TANGGAL_OPEN_PRICE,
				B.RANGE_BW,
				B.REG_EMP AS EMPLOYEE_ID
			FROM
			...
			WHERE
				...
				(A.PRICE_AREA_CODE = D.CODE AND D.HEAD_CODE = 'TR20') AND
				(A.REQ_NO = '$req_no' AND A.SEQ = '$seq')
		";
		$openprice_data = $CI->db->query($query_openprice)->row_array();

		$current_approval_level = $openprice_data['CURRENT_APPROVAL_LEVEL'];
		$approval_id = $openprice_data['EMPLOYEE_ID'];
		if ($current_approval_level >= 2) {
			$query_latest_approval = "
				SELECT 
					REQ_NO, SEQ, APPROVAL_LEVEL, EMPLOYEE_ID
				FROM TR_SS_ORDER_REQUEST_APPROVAL
				WHERE
					REQ_NO = '$req_no' AND
					SEQ = '$seq' AND
					APPROVAL_LEVEL <= $current_approval_level
			";
			$check_latest_approval = $CI->db->query($query_latest_approval)->row_array();
			
			if (!empty($check_latest_approval)) {
				$approval_id = $check_latest_approval['EMPLOYEE_ID'];
			}
		}
		$query_sales_approval_list = "
			SELECT 
					A.EMPLOYEE_ID,
					A.APPROVAL_LEVEL,
					B.FULL_NAME AS EMPLOYEE_NAME,
					NVL(B.PHONE_NUMBER, '6283807164451') AS PHONE_NUMBER
			FROM
					CD_SALES_APPROVAL A,
					CD_USER B
			WHERE
					A.EMPLOYEE_ID = B.EMPLOYEE_ID AND
					A.EMPLOYEE_APPROVAL_ID = '$approval_id' AND
					A.APPROVAL_LEVEL = '$current_approval_level'
		";
		$sales_approval_list = $CI->db->query($query_sales_approval_list)->result_array();
		// dd($sales_approval_list);
		if (!empty($sales_approval_list)) {
			$insert_data = [];
			foreach ($sales_approval_list as $list) {
				$curr_data = [
					"NAMA"					=> $list['EMPLOYEE_NAME'],
					"CUSTOMER_NAMA"	=> $openprice_data['CUSTOMER'].' - '.$openprice_data['CUSTOMER_NAME'],
					"TANGGAL"				=> date('d M Y', strtotime($openprice_data['TANGGAL_OPEN_PRICE'])),
					"RANGE_BW"			=> $openprice_data['RANGE_BW'],
					"OPEN_PRICE"		=> $openprice_data['OPEN_PRICE'],
					"AREA"					=> $openprice_data['PRICE_AREA'],
					"PRICE"					=> $openprice_data['UNIT_PRICE'],
					"LINK_BROILER"	=> LINK_BROILER_WHATSAPP,
					"LINK_APPROVAL"	=> LINK_BROILER_APPROVAL_WHATSAPP,
					"TOKEN"					=> "",
					"STATUS"				=> "N",
					"CREATED_AT"		=> date('Ymd His'),
					"NOMOR_HP"			=> $req_no == '20250520RQ0078' ? '6283807164451' : $list['PHONE_NUMBER']
				];

				$insert_data[] = $curr_data;
			}
			$cjwa_db = $CI->load->database('cjwa', TRUE);
			$save = $cjwa_db->insert_batch('ORDER_APPROVAL', $insert_data);
		}
	}

	function getSalesManID($userLevel, $employeeID, $return = 'AS_LIST') {
		$CI = &get_instance();
		dd($userLevel, FALSE);
		dd($employeeID, FALSE);
		dd($return);
